...
class l1b(initL1b):
    ...
    def processModule(self):
        ...
        for band in self.globalConfig.bands:
            ...
            if self.l1bConfig.do_equalization:
                ...
                eq_mult = readFactor(
                    os.path.join(self.auxdir, self.l1bConfig.eq_mult + band + NC_EXT),
                    EQ_MULT,
                )
                eq_add = readFactor(
                    os.path.join(self.auxdir, self.l1bConfig.eq_add + band + NC_EXT),
                    EQ_ADD,
                )
            ...
            # -------------------------------------------------------------------------------
            self.logger.info(
                "EODP-ALG-L1B-1020: Absolute radiometric gain application (restoration)"
            )
    ...
    def restoration(self, toa, gain):
        ...
        # TODO
        self.logger.debug(
            "Sanity check. TOA in radiances after gain application "
            + str(toa[1, -1])
            + " [mW/m2/sr]"
        )
